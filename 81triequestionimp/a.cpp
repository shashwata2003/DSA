...
public:
    ...
    void printsuggestion(trienode* curr, vector<string>& temp, string prefix) {
        // the node is the terminal
        if (curr->isTerminal) {
            temp.push_back(prefix);
        }
        for (char ch = 'a'; ch <= 'z'; ch++) {
            // looping all the letters by 26 letter 
            trienode* next = curr->child[ch - 'a'];
            if (next != NULL) {
                // next node is present 
                prefix.push_back(ch);
    ...
    vector<vector<string>> getsuggestion(string str) {
        ...
        for (int i = 0; i < str.size(); i++) {
            ...
            vector<string> temp;
            //recursion
            // temp has all element from the curr node
            printsuggestion(curr, temp, prefix);
            output.push_back(temp);
            temp.clear();
            // setting the prev to curr
            prev = curr;
        }
        return output;
    }
...
vector<vector<string>> phonedirectory(vector<string> arr, string str) {
    trie* root = new trie();
    // insert all the word 
    for (int i = 0; i < arr.size(); i++) {
        root->insertword(arr[i]);
    }
    return root->getsuggestion(str);
}
